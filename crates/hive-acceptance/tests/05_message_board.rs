...
async fn topic_create_list_get() {
    ...
    let res = call(&mut ws, "topic.create", json!({
        "title": "My Topic",
...
async fn topic_list_since_filter() {
    ...
    call(&mut ws, "topic.create", json!({"title": "Old Topic", "content": "old"})).await;
    call(&mut ws, "topic.create", json!({"title": "New Topic", "content": "new"})).await;
...
async fn topic_get_since_filter() {
    ...
    // Add 2 comments
    call(&mut ws, "topic.comment", json!({"topic_id": &topic_id, "content": "First"})).await;
    call(&mut ws, "topic.comment", json!({"topic_id": &topic_id, "content": "Second"})).await;
...
async fn topic_wait_resolves_on_new_comment() {
    ...
    tokio::spawn(async move {
        ...
        call(&mut commenter, "topic.comment", json!({
    ...
    assert!(res.error.is_none(), "wait should resolve before timeout: {:?}", res.error);
    let result = res.result.unwrap();
    let comments = result["comments"].as_array().unwrap();
