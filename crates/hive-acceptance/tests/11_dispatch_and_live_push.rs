...
async fn tagged_task_dispatched_to_matching_registered_agent() {
    ...
    assert!(
        push.is_some(),
        "kilo agent should receive task.assign for kilo-tagged task"
    );
    ...
    assert!(
        no_push.is_none(),
        "claude agent should NOT receive kilo-tagged task"
    );
}
...
async fn claude_tagged_task_dispatched_to_claude_agent() {
    ...
    assert!(
        push.is_some(),
        "claude agent should receive task.assign for claude-tagged task"
    );
    let task = &push.unwrap()["params"]["task"];
    ...
    assert!(
        no_push.is_none(),
        "kilo agent should NOT receive claude-tagged task"
    );
}
...
async fn untagged_task_dispatched_to_registered_agent() {
    ...
    assert!(
        push.is_some(),
        "registered agent should receive task.assign for untagged task"
    );
    let task = &push.unwrap()["params"]["task"];
...
async fn push_send_delivers_live_push_notify() {
    ...
    assert!(
        push.is_some(),
        "connected recipient should receive push.notify in real-time"
    );
    ...
    assert!(
        !messages.is_empty(),
        "push.notify should contain at least one message"
    );
...
async fn multi_agent_topic_read_comment_create() {
    ...
    assert!(topics
        .iter()
        .any(|t| t["title"] == "Integration Test Topic"));
