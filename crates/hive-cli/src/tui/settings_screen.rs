...
fn render_config(f: &mut Frame, area: Rect, config: Option<&Config>) {
    ...
    match config {
        ...
        Some(cfg) => {
            let lines = vec![
                Line::from(vec![
                    Span::styled("Project ID:    ", Style::default().add_modifier(Modifier::BOLD)),
                    Span::raw(&cfg.project_id),
                ]),
                Line::from(vec![
                    Span::styled("Server port:   ", Style::default().add_modifier(Modifier::BOLD)),
                    Span::raw(cfg.server.host_port.to_string()),
                ]),
                Line::from(vec![
                    Span::styled("DB path:       ", Style::default().add_modifier(Modifier::BOLD)),
                ...
                Line::from(vec![
                    Span::styled("Log level:     ", Style::default().add_modifier(Modifier::BOLD)),
...
fn render_api_keys(f: &mut Frame, area: Rect) {
    ...
    let lines: Vec<Line> = keys
        ...
        .map(|(var, desc)| {
            ...
            Line::from(vec![
                Span::styled(format!("{var:<22}"), Style::default().add_modifier(Modifier::BOLD)),
        ...
        .collect();
    f.render_widget(
        Paragraph::new(lines).block(Block::default().title("API Keys (from environment)").borders(Borders::ALL)),
        area,
    );
}
...
fn render_docker_controls(f: &mut Frame, area: Rect) {
    let lines = vec![
        Line::from(Span::styled("Docker Controls", Style::default().add_modifier(Modifier::BOLD))),
        Line::from(""),
        Line::from(vec![
